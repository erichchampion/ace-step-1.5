...
| **APG double-epsilon** | **Fixed** | `APG.swift` had epsilon inside `sqrt(diffSq + 1e-8)` AND in `normThreshold / (diffNorm + 1e-8)`. Python only adds epsilon in the denominator. Fixed: removed epsilon from inside `sqrt()`. |
